#############################################################################
#                                                                           #
#   processing microbeJ binary segmented microscopy data combined version   #
#                                                                           #
#############################################################################
...
#### import .csv to global environment ####

temp = list.files(pattern="*.csv")
#fov = paste("F",1:length(temp),sep="") 
for (i in 1:length(temp)) assign(temp[i], read.csv(temp[i]))

#### split master dataframe into F1, F2, F3 etc. and make list ####

F <- get(temp[1]) %>% group_by(EXPERIMENT) %>% group_split()
fov = paste("F",1:length(F),sep="") 
names(F) = fov
